...
api.interceptors.request.use((config) => {
  ...
  if (import.meta.env.DEV) {
    ...
    if (config.params) console.log('Params:', maskSensitiveData(config.params));
    if (config.data) console.log('Payload:', maskSensitiveData(config.data));
    const safeHeaders = maskSensitiveData(config.headers || {});
...
api.interceptors.response.use(
  ...
  async (error) => {
    ...
    if (import.meta.env.DEV) {
      ...
      console.log('Duration:', duration);
      if (error.response?.data) console.log('Response Error Body:', maskSensitiveData(error.response.data));
      console.groupEnd?.();
    }
